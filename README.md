...
- Python 3.8.6
- CUDA 10.1
- 3D Slicer 5.4.0 or newer (see step 1 for installation of python libraries in 3D Slicers.)
- Imagej
...
- •	Crop_to_128x128x64.ijm – Center-cropping 280x200 images to 128x128 images (in Imagej)
- model_3D_unet.py 	
...
- OPI_Rat_3D_NN_Train.py 
  - Training model code
- OPI_Rat_3D_NN_Apply.py 
  - Apply trained weights to images
- BatchSegmentationComparisonMetrics.py (in 3D Slicer)
## Input Data for neural network:
-	MRI Images (Matrix Size: 128x128x64)
-	Matching Labels (Matrix Size: 128x128x64)
...
![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readme/Picture1.png)

  2. Click inside the python console at the bottom of the window (see red arrow below)

![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readme/Picture2.png)
...
![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readme/Picture3.png)
...
3. Create a folder for training output (i.e. U-Net Model Output)
### 6.	Set Up of Training Parameters and Transfer Learning weights(optional):
1. Open OPI_Rat_NN_Train.py in a python editing software (IDLE, Visual Studio Code, etc.)
2. Set path for folders on lines 497 through 531.
3. If you want to utilize transfer learning, set variable PreTrainedWeightsAvailable to True on line 510. Then, set the path to the where the pretrained neural network weights are located. *Note: The weights must match the neural network structure of the 3D U-Net.
4. The code will read all of the “.nii” and “.label.nii” files in the specified folders, sort them from A-Z, and will cycle through that list. *Make sure that MR and Label files are in the same order or else that program will match the wrong MRI and label files together! 
5.	Starting at line 544, uncomment parameters that you would like to change. Default values are what are specified in the paper and are located on lines 92-116.
6.	Once all declared variables are set, you are ready to train.
...
![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readme/Picture4.png)
...
![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readmee/Picture5.png)

3.	Type “Segment Comparison” to find the module and double click on the name:

![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readme/Picture6.png)

4.	This is what the module should look like:

![alt text](https://github.com/ajchaudhari/OPI_Rat_3D_NN/blob/main/Images%20for%20readme/Picture7.png)
