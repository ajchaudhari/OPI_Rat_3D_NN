## 7-14-25 Chaudhari Lab, UC Davis, Davis, CA
## by Valerie Porter

#To Run Code:
# in Windows command console: "path\to\Slicer.exe" --python-script "path\to\BatchN4ITKBiasCorrection.py"
#Example: "C:\Users\Chaudhari\AppData\Local\slicer.org\Slicer 5.4.0\Slicer.exe" --python-script "C:\Users\Chaudhari\Desktop\Batch Codes for 3D Slicer\BatchN4ITKBiasCorrection_v2.py"
...
path2images = 'path\to\images_directory'

#Declare Mask Image(s) Path: if no masks are needed, then declare MaskAvailable variable as False
path2masks = 'path\to\mask_directory'

#Declare Save/Output Path:
path2output = 'path\to\output_directory'

MaskAvailable = False #Parameter for whether a mask is available for bias correction
BiasFieldImage = False #Parameter for whether you would like to save the output bias field calculated.
